...
#define EXPORT_FUNC(a, b, c, ...)
	...
	EXPORT_FUNC(find_cmd, cmd_tbl_t *, find_cmd, const char *cmd)
	EXPORT_FUNC(do_fat_fsload, int, do_fat_fsload, cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
	EXPORT_FUNC(memcpy, void *, memcpy, void *,const void *,__kernel_size_t)
	EXPORT_FUNC(do_fpga, int, do_fpga, cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
	EXPORT_FUNC(timer_get_us, unsigned long, timer_get_us, void)
